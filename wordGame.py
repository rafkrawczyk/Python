...
def getWordScore(word, n):
    ...
    returns: int >= 0
    """
    score = sum(SCRABBLE_LETTER_VALUES.get(ch, 0) for ch in word.lower())
    score *= len(word)
    if len(word) == n:
        score += 50  # bonus for using all letters
    return score
...
# Convert word to lowercase for consistency
    ...
    # Make a copy of the hand so we don't mutate it
    handCopy = hand.copy()

    # Check that the hand has enough of each letter
    for letter in word:
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while calculateHandlen(hand) > 0:
        ...
        # If the input is a period, end the game
        if word == '.':
